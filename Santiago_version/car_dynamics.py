...
class ObjectMovement:
    def __init__(self, state,kinematics):
        self.x = state[:,0:1]
        self.y = state[:,1:2]
        self.speed = kinematics[:,0:1]
        self.angle = kinematics[:,1:2]
    ...
    def dynamics(self, u, PD):
        delta_speed_nn=u[:,0:1]
        delta_angle_nn=u[:,1:2]
        delta_speed_P=PD[:,0:1]
        delta_angle_P=PD[:,1:2]
        ...
        new_position=torch.cat((new_x,new_y),1)
        new_kinematics=torch.cat((self.speed,self.angle),1)
