...
# Fixed starting points for car 1
x_start_car1 = torch.tensor([-3.0])  # Replace fixed_x_start_car1 with your desired value
y_start_car1 = torch.tensor([3.0])  # Replace fixed_y_start_car1 with your desired value
x_end_car1 = torch.tensor([3.0])
y_end_car1 = torch.tensor([-3.0])
...
# Same for car 2
# Fixed starting points for car 2
x_start_car2 = torch.tensor([-3.0])  # Replace fixed_x_start_car2 with your desired value
y_start_car2 = torch.tensor([-3.0])  # Replace fixed_y_start_car2 with your desired value
x_end_car2 = torch.tensor([3.0])
y_end_car2 = torch.tensor([3.0])
...
for i in range(100):
    ...
    for j in range(TrajectoryLength):
        ...
        loss_car1 += criterion(x_car1, y_car1, x_end_car1, y_end_car1)
        loss_car2 += criterion(x_car2, y_car2, x_end_car2, y_end_car2)

    loss_car1 /= TrajectoryLength
    loss_car2 /= TrajectoryLength

    # Perform gradient descent for car 1
    optimizer.zero_grad()
    loss_car1.backward
    optimizer.step()

    #Perform gradient descent for car 2
    optimizer.zero_grad()
    loss_car2.backward
    optimizer.step()

    print("Total Loss for car 1:", loss_car1)
    print("Total Loss for car 2:", loss_car2)
    if (i%20 == 0) :
        # Plot the trajectory
        plt.plot(x_trajectory_car1.detach().clone().numpy(), y_trajectory_car1.detach().clone().numpy(), marker='o')  # 'o' indicates points on the trajectory 
        plt.plot(x_trajectory_car2.detach().clone().numpy(), y_trajectory_car2.detach().clone().numpy(), marker='o')
        plt.plot(x_start_car1.detach().clone().numpy(),y_start_car1.detach().clone().numpy(),'r',marker='x')
        plt.plot(x_start_car2.detach().clone().numpy(),y_start_car2.detach().clone().numpy(),'b',marker='x')
        plt.plot(x_end_car1.detach().clone().numpy(),y_end_car1.detach().clone().numpy(),'r',marker='*')
        plt.plot(x_end_car2.detach().clone().numpy(),y_end_car2.detach().clone().numpy(),'b',marker='*')
